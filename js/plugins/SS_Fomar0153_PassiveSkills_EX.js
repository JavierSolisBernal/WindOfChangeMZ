 ...
 * @version 1.0
 * @base Fomar0153_PassiveSkills
 * @help
 * It is plug and play
 * Fix the bars if TP bar is used
 */
...
(() => {
    ...
    if (!Fomar) {
        console.warn("Fomar0153_PassiveSkills not installed — extension disabled.");
        return;
    }

    
    Window_StatusBase.prototype.placeBasicGauges = function (actor, x, y) {
        Fomar.PassiveSkills.Window_StatusBase_placeBasicGauges.call(this, actor, x, y - 10);
        if (!$gameParty.inBattle()) {
            this.placeGauge(actor, "pp", x + Fomar.PassiveSkills.PPBarOffsetX, y + this.gaugeLineHeight() * 2 + Fomar.PassiveSkills.PPBarOffsetY+1);
        }
    };
